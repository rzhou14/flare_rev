...
sugm.clime.ladm.scr <- function(y_Rachel, Sigma, lambda, nlambda, n, d, maxdf, rho, shrink, prec, max.ite, verbose){
  
  
  q_rachel = dim(as.matrix(y_Rachel))[2]
  ...
  for(j in 1:q_rachel){
    ...
    str=.C("sugm_clime_ladm_scr", as.double(y_vector), as.double(Sigma), as.double(SS.order),
           as.double(icov0), as.double(x0), 
    ...
    icov = matrix(unlist(str[3+1]), byrow = FALSE, ncol = nlambda)
    ...
    x[(col.cnz[j]+1):col.cnz[j+1]] = unlist(str[4+1])[1:cnt]
    row.idx[(col.cnz[j]+1):col.cnz[j+1]] = unlist(str[11+1])[1:cnt]
    ite.int[j,] = unlist(str[12+1])
    ite.int1[j,] = unlist(str[13+1])
    ite.int2[j,] = unlist(str[14+1])
  }
