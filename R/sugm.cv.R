...
sugm.cv <- function(y_Rachel, obj, loss=c("likelihood", "tracel2"), fold=5) {
  x = obj$data
  ...
  for (i in 1:fold) {
    ...
    sugm.cv = sugm(y_Rachel, x.train, lambda=obj$lambda, method=obj$method,sym=obj$sym,verbose=obj$verbose,
